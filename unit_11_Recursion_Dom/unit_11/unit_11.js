...
function t2(elem) {
    ...
    function r2(element) {
        if (!isNaN(element.textContent)) {
            s += Number(element.textContent);
        }
        let tree = element.children;
        Array.from(tree).forEach(elem => r2(elem));
...
// Task 3
// При нажатии кнопки запускается функция t3. 
// Функция t3, с помощью рекурсивной функции r3 должна очистить текст внутри z-3 от тегов span, b, i и p. 
// Должен остаться только текст со знаками препинания в переменной out. Функция t3 должна возвращать out.
...
function t3(elem) {
    ...
    function r3(element) {
            ...
            if(element.firstElementChild == null & element.textContent.includes(',')){
            ...
            let tree = element.children;
        Array.from(tree).forEach(elem => r3(elem));
...
// Task 4
// При нажатии кнопки запускается функция t4. Функция t4, 
// с помощью рекурсивной функции r4 должна тем тегам внутри z-4, 
//которые не имеют вложенных тегов добавить style.background = orange. 
...
function t4(elem) {
    ...
    function r4(element) {
        ...
        let tree = element.children;
        Array.from(tree).forEach(elem => r4(elem));
...
// Task 5
// При нажатии кнопки запускается функция t5. Функция t5, с помощью рекурсивной функции r5 должна те теги внутри z-5, 
// которые не имеют вложенных тегов заменить на теги mark. 
...
function t5(elem) {
    ...
    function r5(element) {
        ...
        let tree = element.children;
        Array.from(tree).forEach(elem => r5(elem));
