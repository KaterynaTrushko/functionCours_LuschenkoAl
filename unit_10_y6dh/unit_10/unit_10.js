...
function t3(arg) {
    ...
    function t4() {
        out = randomInteger(0, 100);
        if (arg == 'odd' & out % 2 == 1) return;
        if (arg == 'even' & out % 2 == 0) return;
        t4();
    }
    t4();
    return out;
...
function t4() {
    ...
    if (ar4.includes(a) == true) t4();
    if (ar4.includes(a) == false) return a;
...
function t5() {
    ...
    ar5.push(num);
    let sum = ar5.reduce((accum, elem) => accum = accum + elem);
    if (sum < 30) {
        t5();
    }
...
document.querySelector('.b-6').addEventListener('click', () => {
    t6(ar6);
    document.querySelector('.out-6').innerHTML = ar6_res;
});
...
function t8() {
    let str = (randomInteger(1000, 9000)).toString();
    let arr = Array.from(str.split(''));
    let arrNew = arr.map(elem => Number(elem));

    if (arrNew[0] + arrNew[1] == arrNew[2] + arrNew[3]) {
...
document.querySelector('.b-10').addEventListener('click', () => {
    ...
    document.querySelector('.out-10').textContent = ar10_res;
});